...
import { getBrands, type Brand } from '../data/apiService';
...
export const Brands: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    const status = form.status as 'Active' | 'Inactive';
    if (editing) {
      setBrands(prev => prev.map(b => b.id === editing.id ? { ...b, name: form.name, country: form.country, status } : b));
      showToast('Brand updated!');
    } else {
      setBrands(prev => [...prev, { id: Date.now(), productCount: 0, name: form.name, country: form.country, status }]);
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {loading ? (
                ...
                <tr key={brand.id} className="hover:bg-[var(--color-surface-secondary)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-[var(--color-text-secondary)]">{brand.country || 'N/A'}</td>
