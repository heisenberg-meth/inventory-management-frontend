...
import { getPayments, type Payment, getDashboardStats, type DashboardKPIs } from '../data/apiService';
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-[var(--color-text-secondary)]">No payments found</td></tr>
              ) : payments.filter(p => activeTab === 'All Payments' || p.status === activeTab).map((payment) => (
                <tr key={payment.id} className="hover:bg-[var(--color-surface-secondary)] transition-colors group">
                  <td className="px-4 py-4 whitespace-nowrap">
                    <span className="text-sm font-medium text-[var(--color-mint)]">PAY-{payment.id}</span>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
                    <span className="text-sm font-medium text-[var(--color-mint)]">{payment.invoiceNumber}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-sm text-[var(--color-text-primary)]">{payment.customerName}</span>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <div className="flex flex-col">
                      <span className="text-sm font-semibold text-[var(--color-text-primary)]">₹{payment.amount.toLocaleString()}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold text-white rounded-md ${
                      payment.status === 'COMPLETED' ? 'bg-[var(--color-mint)]' : 
                      payment.status === 'PENDING' ? 'bg-[var(--color-warning)]' : 'bg-[var(--color-danger)]'
                    }`}>
                      {payment.status}
                    </span>
                  </td>
