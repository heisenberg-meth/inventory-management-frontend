...
export const Warehouses: React.FC = () => {
  ...
  const [form, setForm] = useState({ name: '', location: '', capacity: '1000', manager: 'System', type: 'Primary', status: 'Active' });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Aggregate products by storage_location
      const locMap: Record<string, Warehouse> = {};
      products.forEach(p => {
        const loc = p.storage_location || 'General Store';
        if (!locMap[loc]) {
          locMap[loc] = {
            id: loc,
            name: loc,
            location: loc,
            capacity: 5000, // Dummy capacity
            used: 0,
            manager: 'System',
            type: 'Primary',
            status: 'Active'
        ...
        locMap[loc].used += p.stock;
      });

      setWarehouses(Object.values(locMap));
    } catch (err) {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      setWarehouses(prev => [...prev, { id: String(Date.now()), used: 0, ...form, capacity: Number(form.capacity) }]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {loading ? (
          <div className="col-span-full py-20 flex flex-col items-center justify-center bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-xl">
            <div className="w-10 h-10 border-4 border-[var(--color-mint)] border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="text-[var(--color-text-secondary)]">Analyzing inventory locations...</p>
          </div>
        ) : warehouses.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-xl">
             <WarehouseIcon className="w-12 h-12 text-[var(--color-text-muted)] mx-auto mb-3 opacity-20" />
             <p className="text-[var(--color-text-secondary)]">No warehouse locations found in product data.</p>
          </div>
        ) : warehouses.map(w => {
