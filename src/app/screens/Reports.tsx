  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[var(--color-text-primary)] mb-1">Reports</h1>
          <p className="text-sm text-[var(--color-text-secondary)]">Detailed analytics and insights</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredReports.map((report, idx) => {
          ...
          return (
            <div key={idx} className="bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-lg p-4 sm:p-6 hover:border-[var(--color-mint)] transition-all hover:shadow-lg">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm sm:text-base font-semibold text-[var(--color-text-primary)] mb-1 leading-snug">{report.name}</h3>
                  <p className="text-xs text-[var(--color-text-secondary)]">{report.description}</p>
                </div>
              </div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-xs text-[var(--color-text-muted)]">Last run: {report.lastRun}</div>
              </div>
      ...
      {/* Featured Analytics */}
      <div>
        <h2 className="text-lg sm:text-xl font-semibold text-[var(--color-text-primary)] mb-4">Featured Analytics</h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          {/* Sales Trend Chart */}
          <div className="lg:col-span-2 bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-lg p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-[var(--color-text-primary)]">Sales Trend (Last 30 Days)</h3>
              <button onClick={() => showSuccess('Chart data exported!')} className="text-xs text-[var(--color-mint)] hover:underline flex items-center gap-1">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={salesTrendData}>
                ...
                <Line type="monotone" dataKey="sales" stroke="var(--color-mint)" strokeWidth={2} dot={{ fill: 'var(--color-mint)', r: 4 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-lg p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-[var(--color-text-primary)]">Top Products by Revenue</h3>
            </div>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={topProductsData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                <XAxis type="number" stroke="var(--color-text-muted)" fontSize={11} />
                <YAxis dataKey="product" type="category" stroke="var(--color-text-muted)" fontSize={11} width={75} />
                <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface-secondary)', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-text-primary)' }} />
                <Bar dataKey="revenue" fill="var(--color-mint)" radius={[0, 8, 8, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-[var(--color-border)]">
              <div>
                <h2 className="text-xl font-semibold text-[var(--color-text-primary)]">{viewReport.name}</h2>
                <p className="text-sm text-[var(--color-text-secondary)]">Generated just now</p>
              </div>
              <button onClick={() => setViewReport(null)} className="p-2 rounded-lg hover:bg-[var(--color-surface-secondary)] text-[var(--color-text-secondary)] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6">
              <div className="space-y-3">
                {[
                  { label: 'Report Period', value: dateRange === 'last7' ? 'Last 7 days' : dateRange === 'last30' ? 'Last 30 days' : dateRange === 'last90' ? 'Last 90 days' : 'This Year' },
                  { label: 'Category', value: viewReport.category },
                  { label: 'Total Records', value: '1,248' },
                  { label: 'Total Value', value: '₹4,52,000' },
                ].map(item => (
                  <div key={item.label} className="flex justify-between items-center py-2 border-b border-[var(--color-border)]">
              ...
              <div className="mt-6 p-4 bg-[var(--color-mint-glow)] border border-[var(--color-mint)]/30 rounded-lg">
                <p className="text-sm text-[var(--color-mint)]">✓ Report generated successfully. Download to view full data.</p>
              </div>
            </div>
            <div className="p-6 border-t border-[var(--color-border)] flex gap-3 justify-end">
              <button onClick={() => setViewReport(null)} className="px-4 py-2 bg-[var(--color-surface-secondary)] border border-[var(--color-border)] text-[var(--color-text-primary)] rounded-lg transition-colors text-sm">Close</button>
              <button 
                onClick={() => { showSuccess(`${viewReport.name} downloaded!`); setViewReport(null); }}
