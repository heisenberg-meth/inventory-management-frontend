...
const actionColor = (type: string) => {
  switch (type) {
    case 'create': return 'bg-[var(--color-mint)]/20 text-[var(--color-mint)]';
    case 'update': return 'bg-[var(--color-info)]/20 text-[var(--color-info)]';
    case 'delete': return 'bg-[var(--color-danger)]/20 text-[var(--color-danger)]';
    case 'export': return 'bg-[var(--color-warning)]/20 text-[var(--color-warning)]';
    case 'auth': return 'bg-purple-500/20 text-purple-400';
...
export const ActivityLog: React.FC = () => {
  ...
  const filtered = ACTIVITY_LOGS.filter(log => {
    const matchSearch = !search || log.action.toLowerCase().includes(search.toLowerCase()) || log.user.toLowerCase().includes(search.toLowerCase()) || log.target.toLowerCase().includes(search.toLowerCase());
        ...
        {[['All', 'Total'], ['create', 'Created'], ['update', 'Updated'], ['delete', 'Deleted'], ['auth', 'Auth']].map(([type, label]) => (
          <button key={type} onClick={() => setTypeFilter(type as string)}
            ...
            <div className="text-lg font-bold mt-0.5">
              {type === 'All' ? ACTIVITY_LOGS.length : ACTIVITY_LOGS.filter(l => l.type === type).length}
            </div>
          </button>
        ))}
      </div>
