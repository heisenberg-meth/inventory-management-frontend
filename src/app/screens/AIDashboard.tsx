...
export const AIDashboard: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #1db97a22, #1db97a44)', border: '1px solid #1db97a44' }}
            >
              <Sparkles className="w-5 h-5" style={{ color: 'var(--color-mint)' }} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #1db97a, #17a068)', borderRadius: '8px' }}
            >
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ backgroundColor: `${card.iconColor}18`, border: `1.5px solid ${card.iconColor}30` }}
                >
                  <Icon className="w-5 h-5" style={{ color: card.iconColor }} strokeWidth={1.8} />
                </div>
                <div className="text-[15px] font-semibold text-[var(--color-text-primary)] leading-tight">{card.title}</div>
                <div className="text-[12px] text-[var(--color-text-muted)] mt-0.5">{card.subtitle}</div>
        ...
        <div className="bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-xl p-5 sm:p-6">
          <div className="flex items-center gap-3 mb-5">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #1db97a18, #1db97a30)' }}
            >
              <Zap className="w-5 h-5" style={{ color: 'var(--color-mint)' }} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[16px] font-bold text-[var(--color-text-primary)]">AI Features Available</span>
              <span
                ...
                {col.map((feat, fi) => (
                  <div key={fi} className="flex items-center gap-2.5">
                    <div
                      ...
                      style={{ backgroundColor: feat.color, boxShadow: `0 0 6px ${feat.color}80` }}
                    />
                    <span className="text-[13px] text-[var(--color-text-secondary)]">{feat.text}</span>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    <TrendingUp className="w-4 h-4" style={{ color: 'var(--color-mint)' }} />
                    <span className="text-[14px] font-semibold text-[var(--color-text-primary)]">Inventory Health Score</span>
                  </div>
                    ...
                    <div className="flex-1 w-full space-y-4">
                      ...
                      <div className="pt-2 flex gap-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[11px] text-[var(--color-text-muted)]">Excellent (≥80%)</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-2 rounded-full bg-[var(--color-warning)]" />
                          <span className="text-[11px] text-[var(--color-text-muted)]">Moderate (&lt;80%)</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <Sparkles className="w-4 h-4" style={{ color: 'var(--color-mint)' }} />
                      <span className="text-[14px] font-semibold text-[var(--color-text-primary)]">AI Recommendations</span>
                    </div>
                    <span
                      className="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide"
                      style={{ background: 'var(--color-mint)22', color: 'var(--color-mint)' }}
                    >
                      Live
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    {RECOMMENDATIONS.map((rec) => (
                      ...
                      >
                        ...
                        >
                          <span style={{ color: urgencyDot(rec.urgency), fontSize: 13 }}>✦</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[13px] font-semibold text-[var(--color-text-primary)] leading-tight">{rec.title}</div>
                          <div className="text-[12px] text-[var(--color-text-muted)] mt-0.5 truncate">{rec.desc}</div>
                        </div>
                        <div className="text-[11px] text-[var(--color-text-muted)] flex-shrink-0 pt-0.5 tabular-nums">{rec.time}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    style={{ color: 'var(--color-mint)', background: 'var(--color-mint-glow)', border: '1px solid var(--color-mint)22' }}
                  >
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { label: 'Predicted Restock Events', value: '7', sub: 'Next 7 days', color: 'var(--color-mint)' },
                    { label: 'Demand Surge Items', value: '3', sub: 'Predicted spike', color: 'var(--color-warning)' },
                    { label: 'AI Accuracy Score', value: '94.2%', sub: 'Model confidence', color: 'var(--color-info)' },
                  ].map((s, i) => (
                    <div key={i} className="rounded-xl p-5" style={{ background: 'var(--color-surface-secondary)', border: '1px solid var(--color-border)' }}>
                      <div className="text-[12px] text-[var(--color-text-muted)] mb-1">{s.label}</div>
                ...
                <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
                  <div className="px-5 py-3 text-[13px] font-semibold text-[var(--color-text-primary)]"
                  ...
                  <div className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
                    {[
                      { product: 'Amoxicillin 250mg',   change: '+40%', status: 'Surge',    color: 'var(--color-warning)' },
                      { product: 'Paracetamol 500mg',   change: '+12%', status: 'Growing',  color: 'var(--color-mint)'    },
                      { product: 'Cetirizine 10mg',     change: '+28%', status: 'Surge',    color: 'var(--color-warning)' },
                      { product: 'Vitamin D3 60K',      change: '+8%',  status: 'Stable',   color: 'var(--color-mint)'    },
                      { product: 'Atorvastatin 20mg',   change: '-5%',  status: 'Decline',  color: 'var(--color-danger)'  },
                    ].map((row, i) => (
                      <div key={i} className="flex items-center justify-between px-5 py-3 hover:bg-[var(--color-surface-secondary)] transition-colors">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-md flex items-center justify-center" style={{ background: 'var(--color-surface-secondary)' }}>
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-[13px] font-semibold tabular-nums" style={{ color: row.color }}>{row.change}</span>
                          <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: `${row.color}18`, color: row.color }}>{row.status}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === 'errors' && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { label: 'Anomalies Detected', value: '3', color: 'var(--color-danger)'  },
                    { label: 'Auto-Resolved',       value: '1', color: 'var(--color-mint)'   },
                    { label: 'Pending Review',      value: '2', color: 'var(--color-warning)' },
                  ].map((s, i) => (
                    <div key={i} className="rounded-xl p-5" style={{ background: 'var(--color-surface-secondary)', border: '1px solid var(--color-border)' }}>
                      <div className="text-[12px] text-[var(--color-text-muted)] mb-1">{s.label}</div>
                      <div className="text-[28px] font-bold" style={{ color: s.color }}>{s.value}</div>
                    ...
                    {[
                      {
                        ...
                        time: '10 mins ago', status: 'Pending', color: 'var(--color-warning)',
                      },
                      {
                        title: 'Negative Stock Count',
                        desc: 'Ibuprofen 400mg — reported stock value is negative (-3)',
                        time: '1h ago', status: 'Pending', color: 'var(--color-danger)',
                      },
                      {
                        title: 'Price Mismatch',
                        desc: 'Cetirizine 10mg — purchase price higher than selling price',
                        time: '4h ago', status: 'Resolved', color: 'var(--color-mint)',
                      },
                    ].map((err, i) => (
                      <div key={i} className="flex items-start gap-3 px-5 py-4 hover:bg-[var(--color-surface-secondary)] transition-colors">
                        <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: err.color }} />
                        <div className="flex-1 min-w-0">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: `${err.color}18`, color: err.color }}>{err.status}</div>
      ...
      >
        <Info className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--color-mint)' }} />
        ...
        <div className="ml-auto flex gap-1 flex-shrink-0">
          {TICKER_EVENTS.map((_, i) => (
            <div
              ...
              style={{ background: i === tickerIdx ? 'var(--color-mint)' : 'var(--color-border)' }}
            />
          ))}
        </div>
      </div>
      ...
      {showSmartSearch && (
        <div className="fixed inset-0 bg-black/60 flex items-start justify-center z-50 p-4 pt-20">
          <div
            ...
            style={{ background: 'var(--color-card-bg)', border: '1px solid var(--color-mint)44' }}
          >
            {/* Input */}
            <div className="flex items-center gap-3 p-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
              <Sparkles className="w-5 h-5 flex-shrink-0" style={{ color: 'var(--color-mint)' }} />
              <input
                autoFocus
                type="text"
              ...
              />
              <button
                onClick={() => { setShowSmartSearch(false); setSmartQuery(''); }}
            ...
            <div className="p-4 space-y-1.5">
              <div className="text-[11px] text-[var(--color-text-muted)] uppercase tracking-wider mb-2 font-semibold">Suggested</div>
              {[
                ...
                >
                  <Sparkles className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--color-mint)' }} />
                  {s}
                </button>
              ))}
            </div>
            {smartQuery && (
              <div className="px-4 pb-4">
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg, #1db97a, #17a068)' }}
                  onClick={() => { setShowSmartSearch(false); setSmartQuery(''); }}
                >
                  Search with AI ✦
                </button>
