...
export const PlatformAdminLayout: React.FC = () => {
  ...
  return (
    <div
      className={`pa-root ${isDark ? "dark" : "light"} flex h-screen w-full overflow-hidden bg-[var(--pa-page-bg)] transition-colors duration-300`}
    >
      ...
      >
        ...
        <div className="h-[72px] flex items-center px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col leading-none">
              <span
                className="text-white text-[14px] font-extrabold tracking-widest"
                style={{
              ...
              <span
                className="text-[#1db97a] text-[9px] font-bold tracking-[0.2em] mt-0.5"
                style={{ WebkitTextStroke: "0.5px rgba(13,110,90,0.5)" }}
              >
                INFO TECH
              </span>
            </div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-1 scrollbar-none">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-[18px] h-[18px] ${active ? "text-[#0d6e5a]" : "text-current opacity-70 group-hover:opacity-100"}`}
                />
                <span className="text-[11px] font-bold uppercase tracking-[0.1em]">
                  {item.label}
                </span>
                {active && (
                  <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#0d6e5a]" />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-6 border-t border-white/5 bg-black/5">
          <div className="flex items-center justify-between">
            <div className="text-[9px] font-extrabold text-white/20 uppercase tracking-[0.2em]">
              PLATFORM v4.2.0
            </div>
            <button
        ...
        <header className="h-[72px] bg-[var(--pa-topbar-bg)] border-b border-[var(--pa-border)] px-10 flex items-center justify-between flex-shrink-0 z-30 shadow-sm">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              ...
              <h2 className="text-[18px] font-extrabold text-[var(--pa-text-near-black)] tracking-tight -mt-0.5">
                {currentHeader.title}
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-[var(--pa-border)]">
              <div className="text-right hidden sm:block">
                <div className="text-[13px] font-semibold text-[var(--pa-text-near-black)]">
                  Admin Root
                </div>
                <div className="text-[10px] font-semibold text-[#1db97a] uppercase tracking-wider">
                  AUTHORIZED
                </div>
              </div>
